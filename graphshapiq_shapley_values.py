...
from shapiq.explainer.graph import GraphSHAPIQ, L_Shapley, _compute_baseline_value, get_explanation_instances
...
def save_results(identifier, model_id, data_id, game, sse, max_neighborhood_size, budget):
    """
    ...
    final_results["budget_LShapley"] = budget["lshap"]
    final_results["budget_GraphSHAP-IQ"] = budget["gshap"]
    ...
    save_path = os.path.join("results/approximation_shapley", save_name + ".csv")
    final_results.to_csv(save_path)


def explain_instances_with_gt(
    model_id, model, all_samples_to_explain, explanation_order=1, masking_mode="feature-removal"
):
    ...
    for data_id, x_graph in enumerate(all_samples_to_explain):
        if not stop_processing:
            ...
            if x_graph.num_nodes <= 12:
                ...
                budget["lshap"] = {}
                budget["gshap"] = {}
                approx_id_lshap = "LShapley"
                approx_id_gshap = "GraphSHAP-IQ"
                sse[approx_id_lshap] = {}
                sse[approx_id_gshap] = {}

                for max_interaction_size in range(1, game.n_players + 1):
                    # Compute the Moebius values for all subsets
                    lshap_sv = lShapley.explain(max_interaction_size)
                    gshap_moebius, gshap_sv = gSHAP.explain(max_interaction_size,explanation_order,True)
                    budget["gshap"][max_interaction_size] = gSHAP.last_n_model_calls
                    budget["lshap"][
                        max_interaction_size
                    ] = lShapley.last_n_model_calls

                    sse[approx_id_gshap][max_interaction_size] = np.sum(
                        (gshap_sv - gt_interaction).values ** 2
                    )
                    sse[approx_id_lshap][max_interaction_size] = np.sum(
                        (lshap_sv - gt_interaction).values ** 2
                    )
                ...
                for approx_id, baseline in BASELINE_APPROXIMATORS.items():
                    ...
                    sse[approx_id] = {}
                    for max_interaction_size, baseline_budget in budget["gshap"].items():
                        baseline_interactions = baseline_approx.approximate(
                            game=game, budget=baseline_budget
                        )
                ...
                save_results(
                    "lShapley", model_id, data_id, game, sse, lShapley.max_size_neighbors, budget
                )
                counter += 1
        if counter >= 5:
...
if __name__ == "__main__":
    DATASET_NAMES = ["Mutagenicity"] #["PROTEINS", "ENZYMES", "MUTAG", "AIDS", "DHFR", "COX2", "BZR", "Mutagenicity"]
    MODEL_TYPES = ["GCN"]
    N_LAYERS = [2,3] #[1, 2, 3, 4]
    EXPLAIN_WITH_GT = True
    EXPLAIN_WITHOUT_GT = False
