...
class GraphSHAPIQ:
    def __init__(self, game: Union[GraphGame, GraphNodeGame]):
        ...
        if self.max_size_neighbors <= 22:
            moebius_interactions, incomplete_neighborhoods = self._get_all_coalitions(max_interaction_size=self.max_size_neighbors,efficiency_routine=False)
    ...
    def _get_neighborhoods(self):
        """ Computes the neighborhoods of each node and caps the max_interaction_size at the size of the largest
        neighborhood.
        ...
        with mp.Pool(self.n_jobs) as pool:
            neighbors_list = list(
                pool.starmap(self._get_k_neighborhood, [(node_id,) for node_id in self._grand_coalition_set]))
    ...
    def _efficiency_routine(self,masked_predictions, moebius_coefficients, incomplete_neighborhoods,incomplete_neighborhoods_lookup,max_interaction_size, _grand_coalition_prediction_node):
    ...
    def explain(
            ...
            moebius_interactions, incomplete_neighborhoods = self._get_all_coalitions(max_interaction_size, efficiency_routine)
            ...
            if self.output_dim == 1:
                moebius_coefficients, shapley_interactions = self._grapshapiq_routine(moebius_interactions,
                                                                                              masked_predictions,
                ...
                for id in range(self.output_dim):
                    ...
                    moebius_coefficients[id], shapley_interactions[id] = self._grapshapiq_routine(moebius_interactions, masked_predictions_current_node, moebius_coalition_lookup,
                                        efficiency_routine, incomplete_neighborhoods, incomplete_neighborhoods_lookup,
                                        max_interaction_size, order, _grand_coalition_prediction_node)

            return moebius_coefficients, shapley_interactions


    def _grapshapiq_routine(self, moebius_interactions, masked_predictions, moebius_coalition_lookup, efficiency_routine, incomplete_neighborhoods, incomplete_neighborhoods_lookup, max_interaction_size, order, _grand_coalition_prediction_node):
        ...
        if efficiency_routine:
            ...
            additional_moebius_coefficients = self._efficiency_routine(masked_predictions, moebius_coefficients,
                                                                       incomplete_neighborhoods,
                                                                       incomplete_neighborhoods_lookup,
                                                                       max_interaction_size, _grand_coalition_prediction_node)
    ...
    def _get_all_coalitions(self, max_interaction_size: int, efficiency_routine: bool) -> (dict, dict):
        """ Collects all coalitions that will be evaluated, i.e. coalitions with non-zero Möbius transform of maximum
        size max_interaction_size. If efficiency_routine is True, then all neighborhoods are added to the collection.
